...
def get_cdf_info(cdf_path, tof_option: TOFOption, wrr, wtof, scanner_option):
    ip_type = [('time', 'i4'), ('castor_id_1', 'i4'), ('castor_id_2', 'i4')]
    if wtof:
        # op_dtype[1:1] = [('tof_resolution', 'f4')]
        ip_type[1:1] = [('tof', 'f4')]
    ...
    rr = cdf["random_rate"] if wrr else np.zeros_like(time)
    tof = cdf["tof"]
    ...
    sino_index, rm_option = crystal_base_id_to_block_base_sinogram(np.column_stack((target_cdf[:, -2], target_cdf[:, -1])))
    # sino_index, rm_option = castor_id_to_sinogram(scanner_option, np.column_stack((castor_id_1, castor_id_2)), False)
    target_cdf = target_cdf[~rm_option, :]

    return target_cdf, sino_index
