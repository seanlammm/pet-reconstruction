...
from gPET_Scatter_Correction.others import crystal_base_id_to_block_base_sinogram
...
def get_scale_by_sinogram_bin_counts_sum_all_tof(target_cdf, gpet_cdf, scan_time, scatter_sinogram, scanner_option: ScannerOption):
    miche_index, _ = crystal_base_id_to_block_base_sinogram(target_cdf[:, -2:])
    target_sinogram = np.zeros([scanner_option.rings, scanner_option.rings, scanner_option.views, scanner_option.bins])
    np.add.at(target_sinogram, tuple(miche_index.astype(int).T), 1)

    miche_index, _ = crystal_base_id_to_block_base_sinogram(gpet_cdf[:, -2:])
    gpet_sinogram = np.zeros([scanner_option.rings, scanner_option.rings, scanner_option.views, scanner_option.bins])
    np.add.at(gpet_sinogram, tuple(miche_index.astype(int).T), 1)
    ...
    sigma = 8 / (2 * np.sqrt(2 * np.log(2)))  # 精确计算，≈3.398
