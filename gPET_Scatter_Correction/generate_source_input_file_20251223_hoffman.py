...
def print_source_file(index, source_particle, location, dir_path, phantom_voxel_size):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        for i in range(source_particle.shape[0]):
            f.write(f"{(source_particle[i]).astype(int)} {0} {0} {location[i, 0]} {location[i, 1]} {location[i, 2]} {phantom_voxel_size[-1]} {phantom_voxel_size[-2]} {phantom_voxel_size[-3]}\n")


def print_input_file(index, dir_path, source_dir, scanner_geo, phantom_dim, phantom_voxel_size, phantom_material_file, phantom_density_file):
    file_path = dir_path + "input_%d.in" % index
    # 定义需要替换的变量（可根据实际值修改）
    device_number = 0
    noncolinear_angle = 0.0037056
    phantom_dimension = np.array([phantom_dim[-1], phantom_dim[-2], phantom_dim[-3]])#(384, 384, 71)
    phantom_size = np.array([phantom_dim[-1]*phantom_voxel_size[-1], phantom_dim[-2]*phantom_voxel_size[-2], phantom_dim[-3]*phantom_voxel_size[-3]])# (49.92, 49.92, 19.809)
    phantom_offset = phantom_size / 2 * -1  # (-24.96, -24.96, -9.9045)
    simulation_history = 0
    use_phase_space_source = 0
    source_file = source_dir + "/sources_%d.txt" % index
    particle_type = 0
    positron_range_consider = 0
    decay_time = (0, 100)
    photon_psf_sphere = (0, 0, 0, 5)
    photon_absorption_energy = 1e3
    detector_geo_file = scanner_geo
...
def run_generation(phantom_dim, phantom_voxel_size, phantom_material_file, phantom_density_file, scanner_geo, source_dir, input_dir, particle_num):
    dx, dy, dz = voxel_center_distance(phantom_dim, phantom_voxel_size)  # [z, y, x]
    ...
    for i in range(99999):
        ...
        tail_judge = np.where(cum_sum > 2 ** 30)[0]  # prevent particle num not exceed 2**32
        unique_limited = int(left_index + 5e4)  # prevent point num not exceed 1e5
        if tail_judge.shape[0] != 0:
            if unique_limited < tail_judge[0]:
                right_index = unique_limited
            else:
            ...
            if unique_limited < particle_num_copy.shape[0]:
                right_index = unique_limited
                splits_index[left_index:right_index] = i
                left_index = right_index
            else:
    ...
    for i in range(splits_index.max() + 1):
        ...
        print_source_file(i, current_slice, np.round(np.column_stack((current_dx, current_dy, current_dz)), 4),
                          source_dir, phantom_voxel_size=phantom_voxel_size)
        print_input_file(index=i, dir_path=input_dir, scanner_geo=scanner_geo, source_dir=source_dir,
                         phantom_dim=phantom_dim, phantom_voxel_size=phantom_voxel_size,
                         phantom_material_file=phantom_material_file, phantom_density_file=phantom_density_file)


if __name__ == "__main__":
    phantom_dim = np.array([300, 300, 300])  # [z, y, x]
    phantom_voxel_size = np.array([0.1, 0.1, 0.1])  # [z, y, x]
    img = np.fromfile("/share/home/lyj/files/11panel_recon/20251223_hoffman/recon_20251223_hoffman_sum_up_5m_events_scit1_scale_by_sinogram_all_bin_sum_in_tof/recon_20251223_hoffman_sum_up_5m_events_scit1_scale_by_sinogram_all_bin_sum_in_tof_it6.img", dtype=np.float32).reshape(phantom_dim).astype(np.float32)
    mumap = np.fromfile("/share/home/lyj/files/11panel_recon/20251223_hoffman/20251223_hoffman_lac_registrated.img", dtype=np.float32).reshape(phantom_dim).astype(np.float32)
    phantom_material_file = "/share/home/lyj/Downloads/gPET_to_SZBL/Example/input/phantom_20251223_hoffman/phantom_index.raw" # int32 material index image(in castor dimension)(index in matter4gpet)
    phantom_density_file = "/share/home/lyj/Downloads/gPET_to_SZBL/Example/input/phantom_20251223_hoffman/phantom_density.raw" # float32 density image(in castor dimension)
    sim_name = "20251223_hoffman_5m_events_scit2"
    scanner_geo = "/share/home/lyj/Downloads/gPET_to_SZBL/Example/input/scanner/PET_11Panel_LD_crystal_base.geo"
    source_dir = "/share/home/lyj/Downloads/gPET_to_SZBL/Example/input/source_" + sim_name + "/"
    input_dir = "/share/home/lyj/Downloads/gPET_to_SZBL/Example/input/input_" + sim_name + "/"
    for i in range(phantom_dim[0]):
        img[i, :, :] = zero_outside_radius(img[i, :, :], radius=110)
    img[mumap < 1e-2] = 0
    img /= np.percentile(img, 99.9)
    particle_num = img / img.sum() * 6e10  # about 1/6000 events in [411, 611]  # 6e11 for 120m in output
    run_generation(phantom_dim, phantom_voxel_size, phantom_material_file, phantom_density_file, scanner_geo, source_dir, input_dir, particle_num)