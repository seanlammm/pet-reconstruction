...
class MLEM(ReconOption):
    ...
    def get_sense_img(self):
        ...
        if np.all(self.mu_map == 0):
            ai = np.ones(self.events_LOR.shape[0])
        else:
    ...
    def set_0_outsize_fov(self, img):
        ...
        mask = (distance <= (self.scanner_option.scanner_radius * 0.9)).astype(int)
    ...
    def get_objection_function_score(self):
        bi = self.projector.projection_forward_lors_wtof(self.ac_map, self.events_LOR[:, 0], self.events_LOR[:, 1], False)
        flag = bi > 0
        score = np.sum(-bi[flag] + self.yi[flag] * np.log(bi[flag]))
        return score
    ...
    def run(self):
        ...
        for i in tqdm(range(iteration), desc="Running iteration..."):
...
if __name__ == "__main__":
    ...
    ac_map = np.ones([300, 300, 300]).transpose([2, 1, 0])
    mu_map = np.fromfile(r"/share/home/lyj/Documents/WBBrain_data_for_mlaa/50million_sim/clear_mumap.raw", dtype=np.float32).reshape(300, 300, 300).transpose([2, 1, 0])
    ...
    mlem = MLEM(
        ...
        output_dir=r"/share/home/lyj/Documents/WBBrain_data_for_mlaa/test_output/tof_mlem_ac_wmask",
