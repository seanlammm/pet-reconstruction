...
class Projector:
    ...
    def projection_forward_lors_wtof(self, img, lor_start_index, lor_end_index, add_psf, splits=1):
        num_each_split = int(lor_end_index.shape[0] / splits)
        forward_img = xp.zeros([lor_end_index.shape[0], self.tof_option.tof_bin_num])
        for s in range(splits):
            start_index = s * num_each_split
            end_index = (s + 1) * num_each_split if (s != (splits - 1)) else lor_end_index.shape[0]
            current_pos_comb = self.recon_option.get_lor_location(lor_start_index[start_index:end_index], lor_end_index[start_index:end_index])
            forward_img[start_index:end_index, :] = parallelproj.joseph3d_fwd_tof_sino(
                xstart=current_pos_comb[:, :3],
                xend=current_pos_comb[:, 3:],
                img=xp.asarray(img),
                img_origin=self.recon_option.img_origin,
                voxsize=self.recon_option.voxel_size,
                tofbin_width=self.tof_option.tof_bin_width_in_mm,
                sigma_tof=self.tof_option.sigma_tof,
                tofcenter_offset=self.tof_option.tof_center_offset,
